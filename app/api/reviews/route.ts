...
export const POST = async (req: Request) => {
    try {
      ...
      if (!reviewComments && !movieId && !rating) { // Changed from '&&' to '||'
        return NextResponse.json({ error: "Invalid Data" }, { status: 422 });
      }
      await connectToDb();
      const movie = await prisma.movie.findFirst({ where: { id: movieId } });
      if (!movie) {
        return NextResponse.json({ message: "Invalid Movie" }, { status: 401 }); // Changed from "Invalid User" to "Invalid Movie"
      }
      const newReview = await prisma.review.create({ data: { reviewComments, movieId, rating, reviewerName} });
      return NextResponse.json({ message: "Review created successfully", newReview }, { status: 201 }); // Added return statement
    } catch (error: any) {
      console.log(error);
      return NextResponse.json({ error: error.message }, { status: 500 });
    } finally {
